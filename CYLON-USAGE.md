...
- **论文图三件套**：`/var/tmp/cylon/anns/exp/e1c_paper/`（`e1c_results.csv` 全点位数据
  ...
  峰值实测 = 理想模型的 96%）。**已入库仓库 `exp/e1c_paper/`**（另含算力敏感性 11 点、
  BI 税 5 点与综合分析 ANALYSIS.md，见 §8.10.6）。
...
### 8.10.6 Type-3 综合分析与论文数据入库（2026-09-06）
- E1'（f 扫描 7 点）+ 算力敏感性（11 点）+ BI 一致性税（Type-2 D1，5 点）三组数据 +
  图 + 脚本 + 分析已入库 **`exp/e1c_paper/`**；`ANALYSIS.md` = 统一模型
  `wall(f,v) = max(f·T_cpu, (1−f)·(T_miss+500·dist·v))` + 三口径拐点闭合 + 设计启示。
- 关键结论：f* = T_cpu/(T_cpu+T_eng) 预测 0.51/实测 0.50，峰值 1.91×、聚合效率 96%；
  引擎 99% miss-bound（纯引擎点）；算力拐点 v* ≈ 44µs/dist（闭式 = miss 时间/dist，
  拟合斜率 2.26ms/ns-旋钮 vs 理论 2.36）；BI 物理范围税 ≤1%（噪声带内，上限模型
  0.014%/job）。**性能由 miss 延迟 + CPU/引擎均衡决定，对器件算力（5 个数量级）与
  一致性协议开销（4 个数量级）双重不敏感。**
